...
for ($i = 1; $i -lt $results.Count; $i++) {
    $r = $results[$i]
    $slowdownVsBaseline = [math]::Round($r.Time_Sec / $results[0].Time_Sec, 2)
    Write-Host "Compression $($r.Compression): $($r.Time_Sec)s ($slowdownVsBaseline x baseline time), $($r.Output_MB)MB ($($r.Size_Reduction_Pct)% smaller), $($r.Throughput_MBps)MB/s" -ForegroundColor White
}
