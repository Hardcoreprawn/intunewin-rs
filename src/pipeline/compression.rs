...
impl StreamingZipWriter {
    fn new(path: &Path) -> Result<Self> {
        let file = File::create(path)
            .map_err(|e| IntunewinError::FileWriteError { path: path.to_path_buf(), source: e })?;
        Ok(Self {
            file,
            entries: Vec::new(),
        })
    ...
    fn write_entry(&mut self, entry: CompressedEntry) -> Result<()> {
        ...
        self.file.write_all(&entry.compression_method.to_le_bytes())?; // Compression method
        ...
        self.file.write_all(&entry.uncompressed_size.to_le_bytes())?;
        self.file.write_all(&(name_bytes.len() as u16).to_le_bytes())?;
    ...
    fn finish(mut self) -> Result<()> {
        ...
        for entry in &self.entries {
            ...
            self.file.write_all(&(entry.local_header_offset as u32).to_le_bytes())?;
            self.file.write_all(name_bytes)?;
        }
        
        let cd_size = self.file.stream_position()
            .map_err(|e| IntunewinError::CompressionError(e.to_string()))? - cd_offset;
        
        // End of central directory record
        self.file.write_all(&0x06054b50u32.to_le_bytes())?;  // Signature
        self.file.write_all(&0u16.to_le_bytes())?;           // Disk number
        self.file.write_all(&0u16.to_le_bytes())?;           // CD start disk
        self.file.write_all(&(self.entries.len() as u16).to_le_bytes())?;
        self.file.write_all(&(self.entries.len() as u16).to_le_bytes())?;
...
pub fn compress_to_inner_zip(
    ...
    if !output_path.exists() {
        std::fs::create_dir_all(output_path)
            .map_err(|e| IntunewinError::FileWriteError { path: output_path.to_path_buf(), source: e })?;
    }
